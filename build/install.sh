...
if [[ ${PYTHON_VERSION} < "3.9.0" ]]; then
  echo "[install] Python version 3.9.0 or higher is required."
  exit 1
fi
...
Now you should access CLI script: $ das_sankhya --help
Alternatively you can access CLI script via poetry run: $ poetry run das_sankhya --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_das_sankhya_COMPLETE=source_bash das_sankhya)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_das_sankhya_COMPLETE=source_zsh das_sankhya)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_das_sankhya_COMPLETE=source_fish das_sankhya)' >> ~/.config/fish/completions/das_sankhya.fish
EOF